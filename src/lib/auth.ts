...
export async function login(userId: string) {
  ...
  (await cookies()).set('session', token, { httpOnly: true, secure: process.env.COOKIE_SECURE === 'true', sameSite: 'lax', path: '/', maxAge: 604800 });
}
export async function getCart() {
  const jar = await cookies();
  let id = jar.get('cart')?.value;
  let cart = id ? await db.cart.findUnique({ where: { id } }) : null;
  if (!cart) { cart = await db.cart.create({ data: {} }); id = cart.id; jar.set('cart', id, { httpOnly: true, sameSite: 'lax', secure: process.env.COOKIE_SECURE === 'true', path: '/', maxAge: 2592000 }); }
  return cart;
}
export async function rateLimit(key: string, max = 10) {
  ...
  if (updated.count > max) throw new Error('تلاش‌های بیش از حد؛ ۱۵ دقیقه دیگر امتحان کنید.');
}
