...
try {
  if (existsSync(backup)) { console.log('Campaign content already applied; preserving CMS edits.'); }
    ...
    await db.$transaction(async tx => {
      ...
      for (const b of banners) await tx.banner.update({ where: { id: b.id }, data: b.placement === 'collection' ? { title: 'لطافتی که می‌درخشد', subtitle: 'کالکشن رز؛ روایتی از نور، رنگ و ظرافت', image: '/images/rose-campaign.png', button: 'کشف کالکشن' } : { title: 'زیبایی در جزئیات است', subtitle: 'قطعه‌هایی برای همراهی با لحظه‌های ساده و عزیز زندگی', image: '/images/rose-ring.png', button: 'زیور خود را پیدا کنید' } });
      await tx.siteSettings.update({ where: { id: 'main' }, data: { heroImage: '/images/rose-hero.png', heroTitle: 'درخشش، به سبک تو', heroSubtitle: 'ظرافتی برای لحظه‌هایی که ماندگار می‌شوند.', heroButton: 'مشاهده کالکشن' } });
    });
